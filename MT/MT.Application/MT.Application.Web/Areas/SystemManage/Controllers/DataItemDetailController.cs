...
namespace MT.Application.Web.Areas.SystemManage.Controllers
{
    ...
    {
        #region 字段、属性
        private readonly IT_DataItemDetailBLL _iT_DataItemDetailBLL;
        public DataItemDetailController(IT_DataItemDetailBLL iT_DataItemDetailBLL)
        {
            this._iT_DataItemDetailBLL = iT_DataItemDetailBLL;
        }
        #endregion
        ...
        protected string GetWhere()
        {
            try
            {
                ...
                if (Request.Params["F_DataItemID"] != null && !string.IsNullOrEmpty(Request.Params["F_DataItemID"].ToString()))
                {
                    sbWhere.AppendFormat(" and F_DataItemID = '{0}' ", Request.Params["F_DataItemID"].ToString());
                }
        ...
        public ActionResult GetForm(string F_IDValue)
        {
            ...
            try
            {
                ...
                model = _iT_DataItemDetailBLL.GetModelByCondition(x => x.F_ID == F_ID);
        ...
        public ActionResult UpdateF_isEnable(Guid F_ID, bool? IsChecked)
        {
            try
            {
                ...
                T_DataItemDetail T_DataItemModel = _iT_DataItemDetailBLL.GetModelByCondition(x => x.F_ID == F_ID);
                ...
                if (_iT_DataItemDetailBLL.Update() > 0)
                {
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public ActionResult SaveForm(string FormType, string F_IDValue, T_DataItemDetail model)
        {
            try
            {
                switch (FormType.ToUpper())
                {
                    case "ADD":
                        ...
                        model.F_isValid = 1;
                        if (_iT_DataItemDetailBLL.Insert(model) > 0)
                        {
                            return Success("添加成功!");
                        }
                        else
